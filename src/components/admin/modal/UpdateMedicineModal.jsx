...
/** ------------ COMPONENT ------------------ **/
function UpdateMedicineModal({ medicineId, isOpen, onClose, onSuccess }) {
  ...
  // Fetch medicine details when modal opens
  useEffect(() => {
    const fetchMedicineDetails = async () => {
      ...
      try {
        ...
        if (response.status === 200 && response.data?.result) {
          const medicine = response.data.result;

          // Transform medicine data to form initial values
          const transformed = {
            medicineName: medicine.medicineName || "",
            originalPrice: medicine.originalPrice || "",
            discount: medicine.discount || "",
            picUrl: medicine.picUrl || "",
            picUrls: medicine.picUrls || [],
            medicineCount: medicine.medicineCount || "",
            availableStatus: medicine.availableStatus ?? true,
            minOrder: medicine.minOrder || "",
            maxOrder: medicine.maxOrder || "",
            unit: medicine.unit || "tablet",
            description: medicine.description || "",
            generics_and_strengths:
              medicine.generics_and_strengths?.map((item) => ({
                generic: item.generic?._id || item.generic || "",
                strength: item.strength || "",
              })) || [{ generic: "", strength: "" }],
            company: medicine.company?._id || medicine.company || "",
            form: medicine.medicine_form?._id || medicine.medicine_form || "",
            category: medicine.category || "",
            unitInfo: medicine.unitInfo || "",
            orderUnit: medicine.orderUnit || "",
          };
        ...
        addToast({
          title: "Error",
          description: err?.message || "Failed to load medicine details",
          color: "danger",
        });
      } finally {
        setLoading(false);
  ...
  async function uploadImages({ singleFile, multiFiles }) {
    ...
    if (singleFile) fd.append("picUrl", singleFile);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-3 sm:p-4 md:p-6 max-h-[90vh] overflow-y-auto">
      <Formik
